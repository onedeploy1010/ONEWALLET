...
export async function POST(
  ...
  try {
    ...
    const response = await fetch(`${ENGINE_URL}/api/v1/engine/transactions/${txId}/cancel`, {
      method: 'POST',
      headers: {
    ...
    console.error('Engine transaction cancel error:', error);
    return NextResponse.json(
      { success: false, error: { code: 'E5000', message: 'Failed to cancel transaction' } },
      { status: 500 }
    );
  }
}
