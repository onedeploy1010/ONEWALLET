...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${ENGINE_URL}/api/v1/engine/access/admins?${queryString}`, {
      headers: {
    ...
    console.error('Engine admins fetch error:', error);
    return NextResponse.json(
      { success: false, error: { code: 'E5000', message: 'Failed to fetch admins' } },
      { status: 500 }
    );
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${ENGINE_URL}/api/v1/engine/access/admins`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json',
    ...
    console.error('Engine admin create error:', error);
    return NextResponse.json(
      { success: false, error: { code: 'E5000', message: 'Failed to add admin' } },
      { status: 500 }
    );
  }
}
